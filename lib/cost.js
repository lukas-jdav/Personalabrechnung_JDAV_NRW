...
customElements.define("cost-item", class extends HTMLElement {
	...
	init(costKind) {
		...
		const update = () => {
			...
			this.recalculate(costKind, params)
		...
		// React immediately to typed input and also to native select/checkbox changes.
		this.shadowRoot.addEventListener("input", update)
		this.shadowRoot.addEventListener("change", update)
...
customElements.define("cost-section", class extends HTMLElement {
	...
	appendRow(costKind) {
		...
		costItem.addEventListener("cost-item-updated", () => {
			syncRowState()
		})
